...
from loss.losses import MSEWithScore
...
class NeuralAttentionIsolationForest(AttentionForest):
    ...
    def _make_nn(self, n_features):
        if self.params.regularization_lambda != 0.0:
            self._n_features = n_features + 1
        else:
    ...
    def _base_fit(self, X, y) -> 'NeuralAttentionForest':
        ...
        if self.params.regularization_lambda == 0:
            self.training_xs = X.copy()
        else:
            score = self.forest.decision_function(X.copy())
            score = np.expand_dims(score, axis=1)
            self.training_xs = np.concatenate([X.copy(), score], axis=1)
    ...
    def _make_loss(self):
        if callable(self.params.loss):
            ...
            #     return torch.nn.MSELoss()
            # else:
            #     return MSEWithScore(self.params.regularization_lambda)
    ...
    def _optimize_weights_unlabeled_end_to_end(self, X) -> 'NeuralAttentionForest':
        ...
        if self.params.regularization_lambda != 0:
            score = self.forest.decision_function(X_train)
            score_val = self.forest.decision_function(X_val)
        ...
        if self.params.regularization_lambda != 0:
            copy_x = X_tensor.detach().numpy()
            copy_x_val = X_tensor_val.detach().numpy()
            score = torch.tensor(self.forest.decision_function(copy_x),
                                 dtype=torch.double,
                                 requires_grad=True)
            score_val = torch.tensor(self.forest.decision_function(copy_x_val),
                                     dtype=torch.double,
                                     requires_grad=True)
            score = score.unsqueeze(1)
            score_val = score_val.unsqueeze(1)

            X_tensor = torch.cat((X_tensor, score), dim=1)
            X_tensor_val = torch.cat((X_tensor_val, score_val), dim=1)
        ...
        for epoch in range(n_epochs):
            ...
            optim.zero_grad()
            # if self.params.regularization_lambda != 0.0:
            #     copy_xs_recons = xs_reconstruction.detach().numpy()
            #     copy_x = X_tensor.detach().numpy()
            #     score = torch.tensor(self.forest.decision_function(copy_xs_recons),
            #                          dtype=torch.double,
            #                          requires_grad=True)
            #     score_y = torch.tensor(self.forest.decision_function(copy_x),
            #                            dtype=torch.double,
            #                            requires_grad=True)
            #     loss = loss_fn(xs_reconstruction, X_tensor, score, score_y)
            # else:
            ...
            with torch.no_grad():
                ...
                # if self.params.regularization_lambda != 0.0:
                #     copy_xs_recons = xs_reconstruction.detach().numpy()
                #     copy_x = X_tensor_val.detach().numpy()
                #     score = torch.tensor(self.forest.decision_function(copy_xs_recons), dtype=torch.double)
                #     score_y = torch.tensor(self.forest.decision_function(copy_x), dtype=torch.double)
                #     val_loss = loss_fn(xs_reconstruction, X_tensor_val, score, score_y)
                # else:

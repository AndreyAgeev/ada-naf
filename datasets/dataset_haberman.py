import pandas as pd
from sklearn.model_selection import train_test_split
import matplotlib.pyplot as plt
from sklearn.preprocessing import StandardScaler, MinMaxScaler

from datasets.basic_dataset import AbstractDataset
from sklearn import preprocessing


class DatasetHaberman(AbstractDataset):
    def __init__(self):
        super().__init__()

    def load(self):
        """https://www.kaggle.com/datasets/gilsousa/habermans-survival-data-set """
        columns = ['age', 'year', 'nodes', 'survival']
        data = pd.read_csv("/Users/andreyageev/PycharmProjects/AttentionBasedIsolationForestScoreFunction/datasets/haberman.csv", header=None, names=columns)
        data['survival'] = data['survival'].map({1: 0, 2: 1})
        # 1 - пациент выжил - нормал 0
        # 2 - пациент умер - аномали 1
        data = data.astype(float)

        data, labels = data.drop("survival", axis=1), data["survival"]
        # scaler = StandardScaler()
        # data = scaler.fit_transform(data)
        self.data = data
        self.labels = labels

        X_train, X_test, y_train, y_test = train_test_split(self.data, self.labels, test_size=0.33, random_state=42)
        self.X_train = X_train
        self.X_test = X_test
        scaler = StandardScaler()
        scaler.fit(self.X_train)
        self.X_train = scaler.transform(self.X_train)
        self.X_test = scaler.transform(self.X_test)
        self.y_train = y_train.values
        self.y_test = y_test.values

    # def cross_validation_split(self, k: int):
    #     X_train, X_test, y_train, y_test = train_test_split(self.data, self.labels, test_size=0.33, random_state=k)
    #     self.X_train = X_train
    #     self.X_test = X_test
    #     scaler = StandardScaler()
    #     scaler.fit(self.X_train)
    #     self.X_train = scaler.transform(self.X_train)
    #     self.X_test = scaler.transform(self.X_test)
    #     self.y_train = y_train.values
    #     self.y_test = y_test.values

    def plot_dataset(self, data, y, save_path):
        pass

    def get_name(self) -> str:
        return "haberman"

